...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--trace-only",
        action="store_true",
        help="Execute tasks and save traces/usage, but skip all grading and score aggregation",
    )
    parser.add_argument("--worker-id", default="", help="Pool worker suffix; gives this worker an isolated agent/workspace")
    parser.add_argument("--benchmark-run-id", default=None, help="Stable internal run ID for resume")
    parser.add_argument("--resume", action="store_true", help="Skip completed tasks from this worker's incremental result")
    parser.add_argument(
        "--no-judge-cache",
        action="store_true",
...
def _compute_efficiency_summary(
    ...
    for entry in task_entries:
        ...
        score = None if grading.get("status") == "skipped" else float(grading.get("mean", 0.0))
        ...
        per_task_efficiency.append(
            {
                ...
                "tokens_per_score_point": round(tot / score, 1) if score and score > 0 else None,
            }
        )

    # Aggregate scores
    all_scores = [float(g["mean"]) for g in grades_by_task_id.values() if "mean" in g]
...
def _log_efficiency_summary(
    ...
    if mean_score is None:
        logger.info("   Mean score: not scored (trace-only)")
    else:
...
def main():
    ...
    run_id = args.benchmark_run_id or _next_run_id(run_root)
    # Reserve a per-run marker even though the agent workspace itself is
    # persistent.  _next_run_id keys off these numeric directories.
    (run_root / run_id).mkdir(parents=True, exist_ok=True)
    skill_dir = skill_root
    worker_suffix = f"-w{args.worker_id}" if args.worker_id else ""
    agent_id = f"bench-{model_slug}{worker_suffix}"
    # Keep one isolated workspace per model. Task fixtures are recreated inside
    # it before every task, while the agent's Skill allowlist stays stable.
    agent_workspace = Path(f"/tmp/pinchbench/{model_slug}{worker_suffix}/agent_workspace")
    ...
    if args.resume and incremental_path.exists():
        try:
            previous = json.loads(incremental_path.read_text(encoding="utf-8"))
            completed = {entry["task_id"] for entry in previous.get("tasks", []) if entry.get("status") == "success"}
            tasks_to_run = [task for task in tasks_to_run if task.task_id not in completed]
            tasks_by_id = {task.task_id: task for task in tasks_to_run}
            logger.info("Resume: skipping %d completed tasks; %d remain", len(completed), len(tasks_to_run))
        except (OSError, json.JSONDecodeError) as exc:
            logger.warning("Resume checkpoint unreadable (%s); running all assigned tasks", exc)
    ...
    def _wait_for_pending_grade() -> None:
        ...
        # The default path grades in the background. Persist the completed
        # task here as well, otherwise incremental results stay empty until
        # the full benchmark has finished.
    ...
    for i, task in enumerate(tasks_to_run, 1):
        ...
        for run_index in range(runs_per_task):
            ...
            logger.info("%s", "=" * 80)
            # Notify the logging proxy of the current task context so it can
            # route API logs to the correct category/task_id.jsonl file.
            try:
                import urllib.request as _urlreq
                import json as _json
                _payload = _json.dumps({"task_id": task.task_id, "category": task.category}).encode()
                _ctrl_req = _urlreq.Request(
                    "http://127.0.0.1:18080/__proxy_control/set_task",
                    data=_payload,
                    headers={"Content-Type": "application/json"},
                    method="POST",
                )
                _urlreq.urlopen(_ctrl_req, timeout=2)
            except Exception as _proxy_err:
                logger.debug("Proxy control endpoint not available: %s", _proxy_err)
            ...
            # (API logs are written directly to openclaw_eval/api_logs/ by the proxy)
        ...
        if args.trace_only:
            ...
            logger.info("⏭️  Task %s: trace captured; grading skipped (--trace-only)", task.task_id)
            _write_incremental_results()
            continue
    ...
    scored_grades = [grade["mean"] for grade in grades_by_task_id.values() if "mean" in grade]
    total_score = sum(scored_grades)
    max_score = float(len(scored_grades))  # Each scored task has max_score of 1.0
    score_pct = (total_score / max_score * 100) if max_score > 0 else 0
    if args.trace_only:
        logger.info("📊 Trace-only run complete: %s tasks captured; no scores computed", len(results))
    else:
