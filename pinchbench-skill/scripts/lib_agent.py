...
def ensure_agent_exists(
    ...
    *,
    ...
    The benchmark agent deliberately has an empty Skill allowlist. This
    agent-local setting leaves the user's normal agents and global Skill
    discovery unchanged.
    ...
    if list_result.returncode == 0:
        ...
        if agent_id.lower() in existing_agents or normalized_id in existing_agents:
            ...
            if (
                ...
                _set_agent_skill_allowlist(agent_id, [])
...
def _set_agent_skill_allowlist(agent_id: str, skills: list[str]) -> bool:
    """Set the final Skill allowlist for one agent without touching globals."""
    try:
        ...
        if result.returncode != 0:
            logger.warning("Unable to inspect agents config: %s", result.stderr.strip())
            return False
        ...
        index = next(
            (
                ...
                if str(entry.get("id", "")).lower() in {agent_id.lower(), normalized_id}
            ),
            None,
        )
        if index is None:
            logger.warning("Unable to find benchmark agent %s in agents.list", agent_id)
            return False
        ...
        if set_result.returncode != 0:
            logger.warning("Unable to set benchmark agent Skill allowlist: %s", set_result.stderr.strip())
            return False
        logger.info("Configured benchmark agent %s with skills=%s", agent_id, skills)
        return True
    except (FileNotFoundError, json.JSONDecodeError, OSError) as exc:
        logger.warning("Unable to configure benchmark agent Skill allowlist: %s", exc)
        return False
...
def prepare_task_workspace(skill_dir: Path, run_id: str, task: Task, agent_id: str) -> Path:
    ...
    # OpenClaw attests an agent workspace after first use. Deleting its root
    # directory makes subsequent turns fail with WorkspaceVanishedError, so
    # clear task artifacts in place and retain the root plus bootstrap files.
    workspace.mkdir(parents=True, exist_ok=True)
    for entry in workspace.iterdir():
        if entry.name in _BOOTSTRAP_FILES:
            continue
        if entry.is_dir() and not entry.is_symlink():
            shutil.rmtree(entry, onerror=_remove_readonly)
        else:
            try:
                entry.unlink()
            except OSError:
...
def execute_openclaw_task(
    *,
    ...
    trace_marker = f"\n\n[PINCHBENCH_TRACE task_id={task.task_id} category={task.category}]"
