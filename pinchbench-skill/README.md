...
### Local clean full run

From the parent `openclaw_eval` checkout, use the project-level clean runner to
run the whole suite with an agent whose final Skill allowlist is empty. It
archives every run by local date and timestamp, and does not upload results by
default:
...
`api_logs/`, and `run.json`. When unspecified, the run ID is a local timestamp
(`YYYYMMDD-HHMMSS`). Add normal PinchBench options after the model, for
example `--suite automated-only`, `--runs 3`, or `--no-parallel-judge`. Use
`--allow-upload` only when a leaderboard submission is intended.
